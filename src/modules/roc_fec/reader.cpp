...
packet::PacketPtr Reader::read_() {
    ...
    if (!started_) {
        packet::PacketPtr pp = read_first_();
...
packet::PacketPtr Reader::read_first_() {
    ...
    if (pp) {
        ...
        resize_maybe_(pp->fec()->source_block_length);
...
void Reader::resize_maybe_(size_t sblen) {
    const size_t prev_sblen = source_block_.size();
    if (prev_sblen == sblen) {
        return;
    }
    ...
    roc_log(LogDebug, "fec reader: update sblen, prev=%lu cur=%lu",
            (unsigned long)prev_sblen, (unsigned long)source_block_.size());
}
...
void Reader::update_source_packets_() {
    ...
    for (;;) {
        ...
        resize_maybe_(fec->source_block_length);
